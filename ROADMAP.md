...
| Docker smoke stack | Live on `127.0.0.1:3100/8100/8110/11435/8889` | `docker-compose.yml` |
...
- `26/26` backend tests pass (`backend/tests/test_*.py`).
...
|---|---|---|---|
| 2026-08-09 | 48 | **Standalone lightweight local-model agent harness.** Added stdlib-only direct Ollama model loop, MCP-derived tool schemas, safe-by-default tool allowlist, bounded turns, in-memory approval resume, structured logs, `/health` `/tools` `/run` `/resume`, Compose service on `:8110`, harness spec, and focused tests. Live acceptance: exact model ready, 14 MCP tools, plain `HARNESS_OK`, safe `search_web`, mutation preview without write, approved resume, guarded cleanup, 26/26 backend tests, frontend typecheck. | `5cbe2b7` |
| 2026-08-09 | 47 | **Local model/tool recovery + structured observability + ACLI.** Restored missing `ChatStreamPayload` import that caused HTTP 422 chat failures; bound structured tools in the HITL loop and fixed ToolNode message feedback; added redacted JSON logs; added `scripts/rws_workflow_cli.py` for coding-agent workspace/workflow modification through approval-gated MCP. Verified direct Ollama and workspace `READY`, 24 ideas, exact model loaded, approved sandbox `5`. | `09605c3` |
| 2026-08-09 | 46 | **MCP agent bridge + expanded runnable workflow library.** Added `POST /mcp` JSON-RPC discovery/resources/prompts/tool calls; 14 bounded tools including approval-gated Python sandbox and workspace file operations; 24 templates with scoped defaults; fixed MCP resource/library/template schema issues; added MCP regression tests. Live acceptance: 22/22 backend tests, frontend typecheck, 14 tools, 3 resources, 24 templates, guarded create/read/patch/read/delete cycle. | `09605c3` |
| 2026-08-09 | 45 | **On-demand Ollama preload/unload.** `GET /api/ollama/ps` (live loaded state) + `POST /api/ollama/preload|unload` (symmetric pair, no approval modal) + Control Center "VRAM · LIVE LOAD STATE" panel polling every 5 s. Exact-model policy still enforced upstream. `host.docker.internal` accepted for both Ollama and SearXNG validator paths; OLLAMA_BASE_URL switched to `http://host.docker.internal:11435` to match the standalone container. | `ba8142c`, `92a55d3` |
